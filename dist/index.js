"use strict";
const PROGRAM_START_TIMER = performance.now();
...
const DISPLAY_TABLE = document.getElementById("table-body");
const NEXT_PAGE_BUTTON = document.getElementById("next-page");
const PREV_PAGE_BUTTON = document.getElementById("prev-page");
const TOTAL_PAGE_NUMBER = document.getElementById("total-page-number");
const CURRENT_PAGE_NUMBER = document.getElementById("current-page-number");
const BROWSE_PAGE_BUTTON = document.getElementById("refresh-page-button");
const SEARCH_PROMPT_INPUT = document.getElementById("search-prompt");
const SEARCH_BUTTON = document.getElementById("start-search");
const SEARCH_OPTION_SELECTION = document.getElementById("search-option-display");
const SORT_BUTTON = document.getElementById("initiate-sort");
const SORT_CATEGORY_SELECTION = document.getElementById("sort-category");
const SORT_ORDER_SELECTION = document.getElementById("sort-order");
...
const DISTANCE_CALCULATOR_BUTTON = document.getElementById("run-distance-calculator");
const DISTANCE_CALCULATOR_OUTPUT = document.getElementById("distance-calculator-output");
const SORT_TIME_DISPLAY = document.getElementById("sort-time-output");
const SEARCH_TIME_DISPlAY = document.getElementById("search-time-output");
const DISTANCE_CALCULATOR_TIME_DISPLAY = document.getElementById("distance-calculator-time-output");
const SHORTEST_PATH_FIRST_AIRPORT_INPUT = document.getElementById("shortest-path-first-airport");
const SHORTEST_PATH_SECOND_AIRPORT_INPUT = document.getElementById("shortest-path-second-airport");
const TRANSITION_NUMBER_INPUT = document.getElementById("transition-number");
const RUN_SHORTEST_PATHFINDING = document.getElementById("search-shortest-path-button");
const SHORTEST_PATH_OUTPUT = document.getElementById("shortest-path-output");
const SHORTEST_PATH_TIME_OUTPUT = document.getElementById("shortest-path-time-output");
const PRORGAM_START_TIME_OUTPUT = document.getElementById("program-start-time-output");
...
let totalNumberDisplayed = NUM_AIRPORTS;
...
function load() {
    ...
    SEARCH_BUTTON.addEventListener("click", () => {
        const PROMPT_INPUT = SEARCH_PROMPT_INPUT.value;
        const SEARCH_CATEGORY = Number(SEARCH_OPTION_SELECTION.value);
    ...
    SORT_BUTTON.addEventListener("click", () => {
        const CATEGORY = Number(SORT_CATEGORY_SELECTION.value);
        const ORDER = Number(SORT_ORDER_SELECTION.value);
        resetData();
    ...
    const PROGRAM_END_TIMER = performance.now();
...
function copyArray(arr) {
    const NEW_ARRAY = Array(arr.length);
 ...
 */
function display(array = displayData, start = displayIndex, length = NUM_DISPLAYED) {
    DISPLAY_TABLE.innerHTML = "";
    const LAST_INDEX_AVAILABLE = displayStartIndex + totalNumberDisplayed;
    const DESIRED_LAST_INDEX_TO_DISPLAY = start + length;
    const INDEX_OF_LAST_ELEMENT_TO_DISPLAY = Math.min(DESIRED_LAST_INDEX_TO_DISPLAY, LAST_INDEX_AVAILABLE);
    for (let i = start; i < INDEX_OF_LAST_ELEMENT_TO_DISPLAY; i++) {
        const NEW_ROW = document.createElement("tr");
        const AIRPORT_TO_DISPLAY = ALL_AIRPORTS[array[i]];
...
function nextPage() {
    const UPPER_BOUND = displayStartIndex + totalNumberDisplayed;
    const NEXT_INDEX_TO_DISPLAY = displayIndex + NUM_DISPLAYED;
...
function previousPage() {
    const PREVIOUS_INDEX_TO_DISPLAY = displayIndex - NUM_DISPLAYED;
 ...
 */
function updatePageFromNumber() {
    const NEW_PAGE_NUMBER = Number(CURRENT_PAGE_NUMBER.value);
    const TOTAL_PAGES = Math.ceil(totalNumberDisplayed / NUM_DISPLAYED);
