 ...
 */
function getDistance(firstAirportId, secondAirportId) {
    const ID_PRESORT = PRESORTED_ARRAYS[0];
    const AIRPORT_ONE = ALL_AIRPORTS[ID_PRESORT[binarySearch(0, firstAirportId)[0]]];
    const AIRPORT_TWO = ALL_AIRPORTS[ID_PRESORT[binarySearch(0, secondAirportId)[0]]];
    const TYPE_INDEX = 2;
    const LATITUDE_INDEX = 5;
    const LONGITUDE_INDEX = 4;
    const LATITUDE_ONE = AIRPORT_ONE[LATITUDE_INDEX];
    const LONGITUDE_ONE = AIRPORT_ONE[LONGITUDE_INDEX];
    const LATITUDE_TWO = AIRPORT_TWO[LATITUDE_INDEX];
    const LONGITUDE_TWO = AIRPORT_TWO[LONGITUDE_INDEX];
    if ((AIRPORT_ONE[TYPE_INDEX] != AIRPORT_TWO[TYPE_INDEX]) || AIRPORT_ONE[TYPE_INDEX] === "closed") {
        return -1;
    }
    const EARTH_RADIUS = 6371;
    const LATITUDE_CALC = Math.pow(Math.sin((degreeToRadian(LATITUDE_TWO - LATITUDE_ONE)) / 2), 2);
    const LONGITUDE_CALC = Math.pow(Math.sin((degreeToRadian(LONGITUDE_TWO - LONGITUDE_ONE)) / 2), 2);
    const DIST = 2 * EARTH_RADIUS * Math.asin(Math.sqrt(LATITUDE_CALC + Math.cos(degreeToRadian(LATITUDE_ONE)) * Math.cos(degreeToRadian(LATITUDE_TWO)) * LONGITUDE_CALC));
    return DIST;
}
 ...
 */
function getCulmulativeDistance(airportIdArray) {
    const DISTANCE_CALCULATOR_START_TIME = performance.now();
    // if there's not enough airports
    if (airportIdArray.length < 2) {
        return [-1, -1];
    }
    const FIRST_AIRPORT_ID = airportIdArray[0];
    const FIRST_AIRPORT = getAirportByID(FIRST_AIRPORT_ID);
    ...
    const TYPE = FIRST_AIRPORT[2];
    let distanceTravelled = 0;
    for (let i = 1; i < airportIdArray.length; i++) {
        const NEW_AIRPORT_ID = airportIdArray[i];
        const PREVIOUS_AIRPORT_ID = airportIdArray[i - 1];
        const NEW_AIRPORT = getAirportByID(NEW_AIRPORT_ID);
        if (NEW_AIRPORT[0] === -1) { // if an airport by that id does not exist
            return [-3, -3];
        }
        const NEW_AIRPORT_TYPE = NEW_AIRPORT[2];
        if (NEW_AIRPORT_TYPE !== TYPE) {
            return [-2, -2];
        }
        const DISTANCE_TO_PREVIOUS_AIRPORT = getDistance(NEW_AIRPORT_ID, PREVIOUS_AIRPORT_ID);
        distanceTravelled += DISTANCE_TO_PREVIOUS_AIRPORT;
    }
    const LAST_AIRPORT_ID = airportIdArray[airportIdArray.length - 1];
    const DISPLACEMENT = getDistance(FIRST_AIRPORT_ID, LAST_AIRPORT_ID);
    const DISTANCE_CALCULATOR_END_TIME = performance.now();
...
function addNewDestination() {
    const NEW_DESTINATION_DISPLAY = document.createElement("div");
    const NEW_INPUT = document.createElement("input");
    const DELETE_BUTTON = document.createElement("button");
    const INPUT_LABEL = document.createElement("h1");
 ...
 */
function calculateDistanceAndDisplayResult() {
    const DESTINATION_INPUTS = document.querySelectorAll(".destination-input");
    const DESTINATIONS_IDS = Array(DESTINATION_INPUTS.length);
    for (let i = 0; i < DESTINATION_INPUTS.length; i++) {
        DESTINATIONS_IDS[i] = DESTINATION_INPUTS[i].value;
    }
    const RESULTS = getCulmulativeDistance(DESTINATIONS_IDS);
    let calculatorOutputMessage;
    if (RESULTS[0] === -1) {
...
//# sourceMappingURL=bonus_one_and_two.js.map