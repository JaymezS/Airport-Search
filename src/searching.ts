 ...
 */
function UserSearchInterface(index: number, key: string): void {
  const SEARCH_START_TIME: number = performance.now();
  ...
  const SEARCH_END_TIME: number = performance.now();
 ...
 */
function binarySearch(index: number, key: string): number[] {

  // get the presorted array (sorted in ascending order) for that category, search in this array
  const ARRAY: number[] = PRESORTED_ARRAYS[index];
  ...
  while (l <= r) {
    ...
    const ELEMENT: string = ALL_AIRPORTS[ARRAY[mid]][index];
