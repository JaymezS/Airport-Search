 ...
 */
function sortInterface(category: number, order: number, array: number[]): number[] {
  const SORT_START_TIME: number = performance.now();
  ...
  const SORT_END_TIME: number  = performance.now();
 ...
 */
function mergeSortTable(
  reference: number[],
  array: number[],
  category: number,
  sortOrder: number = 0,
  start: number = 0,
  length: number = array.length): number[] {
  ...
  let left: number[] = mergeSortTable(reference, array, category, sortOrder, start, mid);
  let right: number[] = mergeSortTable(reference, array, category, sortOrder, start + mid, Math.ceil(length / 2));
...
function mergeTable(
  ...
  leftLength: number,
  right: number,
  rightLength: number): number[] {
  
  // the number of elements that have been merged/used from the left (i) and right (j) side
  let i: number = 0;
  let j: number = 0;

  for (let x: number = 0; x < leftLength + rightLength; x++) {
    const LEFT_AIRPORT_INDEX: number = left + i;
    const RIGHT_AIRPORT_INDEX: number = right + j;
    const LEFT_AIRPORT: any[] = ALL_AIRPORTS[reference[LEFT_AIRPORT_INDEX]];
    const RIGHT_AIRPORT: any[] = ALL_AIRPORTS[reference[RIGHT_AIRPORT_INDEX]];
    const INDEX_TO_CHANGE: number = left + x;
